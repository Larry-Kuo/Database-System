...
public:
  ...
  MyDB_PageReaderWriter(bool pinned, MyDB_TableReaderWriter &parent,
                        int whichPage);
  ...
  // the record is written if there is enough space on the page; otherwise,
  // return a nullptr
  ...
  // by lhs are less than the contens of the record pointed to by rhs...
  // typically, this lambda would have been created via a call to
  // buildRecordComparator
  ...
  void sortInPlace(function<bool()> comparator, MyDB_RecordPtr lhs,
                   MyDB_RecordPtr rhs);
