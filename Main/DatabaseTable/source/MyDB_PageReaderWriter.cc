...
#define NUM_BYTES_USED                                                         \
  *((size_t *)(((char *)myPage->getBytes()) + sizeof(size_t)))
#define NUM_BYTES_LEFT (pageSize - NUM_BYTES_USED)

MyDB_PageReaderWriter ::MyDB_PageReaderWriter(MyDB_TableReaderWriter &parent,
                                              int whichPage) {
  myPage = parent.getBufferMgr()->getPage(parent.getTable(), whichPage);
  pageSize = parent.getBufferMgr()->getPageSize();
}

MyDB_PageReaderWriter ::MyDB_PageReaderWriter(bool pinned,
...
MyDB_PageType MyDB_PageReaderWriter ::getType() { return PAGE_TYPE; }
...
MyDB_PageReaderWriter ::getIterator(MyDB_RecordPtr iterateIntoMe) {
...
void MyDB_PageReaderWriter ::sortInPlace(function<bool()> comparator,
                                         MyDB_RecordPtr lhs,
                                         MyDB_RecordPtr rhs) {
  ...
  // and now we sort the vector of positions, using the record contents to build
  // a comparator
...
MyDB_PageReaderWriterPtr
MyDB_PageReaderWriter ::sort(function<bool()> comparator, MyDB_RecordPtr lhs,
                             MyDB_RecordPtr rhs) {
  ...
  MyDB_PageReaderWriterPtr returnVal =
      make_shared<MyDB_PageReaderWriter>(myPage->getBufferMgr());
  returnVal->clear();
...
size_t MyDB_PageReaderWriter ::getPageSize() { return pageSize; }

void *MyDB_PageReaderWriter ::getBytes() { return myPage->getBytes(); }
