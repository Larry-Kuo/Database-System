...
MyDB_PageHandle MyDB_BufferManager ::getPage(MyDB_TablePtr whichTable,
                                             long id) {
  ...
  if (target != bufferMap.end() && target->second.second->memory) {
    ...
    // add the handler reference to targetPage and make the handler point to the
    // page
    Page targetPage = *itr;
    ...
    if (tableMap.find(whichTable->getName()) == tableMap.end()) {
      tableMap[whichTable->getName()] =
          open(whichTable->getStorageLoc().c_str(), O_RDWR | O_CREAT | O_FSYNC,
               0666);
    }
    // read data from disk
    char data[pageSize];
    ...
    if (currPage < numPages) {
      ...
      if (target != bufferMap.end()) {
        ...
        Page newPage = make_shared<PageBase>(
            bufferPool + offset, whichTable->getName(), id, false, *this);
      ...
      if (target != bufferMap.end()) {
        ...
        Page newPage = make_shared<PageBase>(
            victim->memory, whichTable->getName(), id, false, *this);
        pageContainer.push_back(newPage);
        bufferMap[{whichTable->getName(), id}] = {--pageContainer.end(),
                                                  newPage};
      }
...
MyDB_PageHandle MyDB_BufferManager ::getPage() {
  ...
  if (currPage < numPages) {
    int offset = currPage * pageSize;
    // update the pageContainer
    Page newPage = make_shared<PageBase>(bufferPool + offset, "", anonymousID,
                                         false, *this);
    pageContainer.push_back(newPage);
    // update bufferMap
    ...
    // update the pageContainer
    Page newPage =
        make_shared<PageBase>(victim->memory, "", anonymousID, false, *this);
    pageContainer.push_back(newPage);
    bufferMap[{"", anonymousID}] = {--pageContainer.end(), newPage};
    pageContainer.erase(evictItr);
...
void MyDB_BufferManager ::unpin(MyDB_PageHandle unpinMe) { unpinMe->unpin(); }
...
MyDB_BufferManager ::MyDB_BufferManager(size_t pSize, size_t nPages,
                                        string tempFileName) {
...
MyDB_BufferManager ::~MyDB_BufferManager() {
  ...
  for (std::list<Page>::iterator it = pageContainer.begin();
       it != pageContainer.end(); it++) {
    if ((*it)->dirty) {
      if ((*it)->table != "") {
...
size_t MyDB_BufferManager ::getNumPages() { return numPages; }

void MyDB_BufferManager ::killTable(MyDB_TablePtr killMe) {
  string tableName = killMe->getName();
  if (tableMap.find(tableName) != tableMap.end()) {
    close(tableMap[tableName]);
